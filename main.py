...
class CButton(Button):
  option = ListProperty(["no","yes","upg"])
  pass
...
class XPonerine(ScreenManager):
  ...
  def InitControls(self):
    ...
    idx_app_title = 6
    idx_rec_lable = 4
    self.btnredraw = self.xapptitle.children[idx_app_title].children[0]
    self.btnredraw.bind(on_release=self.RedrawAll)
    self.lblapptitle = self.xapptitle.children[idx_app_title].children[2]
    self.reclabel = self.xapptitle.children[idx_rec_lable].children[0]
    print "InitControls",self.reclabel.text
    ...
    if self.systemmode == 0:
      ...
      self.reclabel.text = 'RECORD TIME'
      self.lblrecordtime.text = self.camrecordtime
    elif self.systemmode == 1:
      self.btnrecord.text = 'PHOTO'
      self.reclabel.text = 'PHOTO COUNT'
      self.lblrecordtime.text = 'P-%04d' %self.camphotocount
  ...
  def DoReadAllStatus(self, index, recstatus):
    ...
    if recstatus in ("record","recording"):
      ...
      cam.CheckSetting("system_default_mode")
      cam.CheckSetting("rec_mode")
      cam.CheckSetting("rec_default_mode")
      cam.CheckSetting("capture_mode")
      cam.CheckSetting("capture_default_mode")
      cam.CheckSetting("buzzer_volume")
      cam.CheckSetting("meter_mode")
      cam.CheckSetting("led_mode")
      cam.CheckSetting("start_wifi_while_booted")
      ...
      if self.buzzervolume != 0:
        threading.Thread(target=self.DoSetConfig, args=(index,"buzzer_volume",forceread,), name="%sDoSetConfig" %cam.name).start()
      # set meter mode
      threading.Thread(target=self.DoSetConfig, args=(index,"meter_mode",forceread,), name="%sDoSetConfig" %cam.name).start()
      # check led mod
      threading.Thread(target=self.DoSetConfig, args=(index,"led_mode",forceread,), name="%sDoSetConfig" %cam.name).start()
      # start wifi on boot
      threading.Thread(target=self.DoSetConfig, args=(index,"start_wifi_while_booted",forceread,), name="%sDoSetConfig" %cam.name).start()
      # set preview status
      threading.Thread(target=self.DoSetConfig, args=(index,"preview_status",forceread,), name="%sDoSetConfig" %cam.name).start()
  ...
  def DoReadAllStatusOld(self, index, recstatus):
    cam = self.cam[index]
    setallok = cam.setallok
    setallerror = cam.setallerror
    t1 = time.time()
    # this config is too slow run in mobile phone
    cam.ReadAllStatus()
    readerror = False
  ...
  def DoConnect(self, index):
    ...
    if self.moveduplicated == 0:
      threading.Thread(target=self.DoReadAllStatusOld, name="%sDoReadAllStatusOld" %cam.name, args=(index,recstatus,)).start()
    else:
    ...
    while not cam.quit.isSet():
      if recstatus in ("record","recording"):
        ...
        if self.recstart < 0: #trigger start by camera
          ...
          if cam.videodelay != 0:
            camdelay = self.delaytime - cam.videodelay
            print "cam %d delay %s system delay %s" %(index, cam.videodelay, self.delaytime)
            print "cam %d need to wait delay time %s" %(index, camdelay)
            if camdelay > 0 and self.delaytime > 0:
              t1 = time.time()
              while (time.time() - t1) < camdelay:
                pass
              print "cam %d record wait delay time %s" %(index, time.time() - t1)
              cam.DebugLog('record_delay %s' %(time.time() - t1))
          cam.StartRecord(False)
  ...
  def CheckDelayTime(self,type=0):
    #type=0: video; type=1: photo
    maxdelay = 0.0
    for cam in self.cam:
      if cam.link:
        print "cam.videodelay %s" %cam.videodelay
        print "cam.photodelay %s" %cam.photodelay
        if type == 0 and cam.videodelay > maxdelay:
          maxdelay = cam.videodelay
        elif type == 1 and cam.photodelay > maxdelay:
          maxdelay = cam.photodelay
    print "CheckDelayTime %s" %maxdelay
    return maxdelay
  
  def Record(self, btnrecord):
    #self.cam[1].GetCamVersion()
    #self.cam[1].CamScriptUpgrade()
    #print "App Version", self.cam[0].GetAppVersion()
    #return
    print "Touch Record Button"
    self.recstart = 0
    self.rename = 0
    self.ftaken = 0
    if btnrecord.text == 'RECORD':
      self.firstcam = 0
      self.recordstop.clear()
      self.delaytime = self.CheckDelayTime(0)
      ...
      self.btnsetting.disabled = True
      self.btnmeter.disabled = True
      self.delaytime = self.CheckDelayTime(1)
      i = 0
      for lbl in self.lblcam:
        if lbl.status == 'link':
          lbl.status = 'busy'
  ...
  def DoCamStatusWait(self, index, format=0, reboot=0):
    ...
    setok = cam.setok
    if format:
      self.lblstatus.color = (1,0,0,0.8)
      self.lblstatus.text = 'FMT'
    ...
    if reboot == 1:
      ...
      self.lblstatus.color = (1,0,0,0.8)
      self.lblstatus.text = 'UPG'
      print "upgrade camera %d" %index
      cam.CamScriptUpgrade()
      cam.getexp.wait()
      if setok.isSet():
        self.lblcam[index].status = 'link'
      else:
...
#         i += 1
    ...
    Clock.unschedule(self.RefreshButtonName)
    Clock.schedule_once(self.RefreshButtonName,0.5)
    ...
    while not cam.quit.isSet():
      ...
      if self.systemmode == 0 and recstatus not in ("record","recording"): #video
        if not self.recordstart.isSet() and self.btnrecord.text == 'RECORD' and cam.recording.isSet():
          print "trigger record start by camera %d" %index, recstatus
          self.recstart = -index
          self.rename = 0
          self.ftaken = 0
          self.recordstop.clear()
