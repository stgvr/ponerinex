...
class XCameraTelnet():
  ...
  def RunCommand(self, cmdlist, msglist, timeout=120):
    self.debuglog = ''
    ...
    if not self.commit.isSet():
      for thread in threading.enumerate():
        if thread.isAlive() and thread.name == tname:
          ...
          self.debuglog += "\nCameraTelnet: kill %s" %thread.name
          try:
  ...
  def DoCommand(self, cmdlist, msglist):
    ...
    self.debuglog += "\nDoCommand start %s" %self.ip
    self.failure = False
    self.retvalue = ''
    self.commit.clear()
    try:
      ret = ""
      tn = telnetlib.Telnet(self.ip, self.port, 60)
      #needs user name
      if self.username <> "":
      ...
      self.debuglog += "\nCameraTelnet Error: %s" %err
      self.failure = True
      pass
  
  def Meter(self, timeout=120):
  ...
  def SetExposure(self, aevalue='', timeout=120):
  ...
  def Rename(self, withpathold, withpathnew, timeout=120):
