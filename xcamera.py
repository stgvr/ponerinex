...
class XCamera():
  ...
  def __init__(self, ip="192.168.42.1", port=7878, dataport=8787, webport=80, enabled=False, preview=False, number=0):
    ...
    #battery time second
    self.battime = 0
    #default remain 60 second
    self.batpercent = 60 
    self.batlast = 0.0
    self.batlastvalue = 0
    
    #delay counter
    self.delaycounter = 0.0
    #photo_capture delay
    self.photodelay = 0.0
    #video_record delay
    self.videodelay = 0.0
...
#     self.trecv.start()
  ...
  def DoBuzzerTelnet(self, uname):
    ...
    cmdlist = ['/tmp/fuse_a/custom/buzzer.sh']
    msglist = ['buzzer600ms']
  ...
  def SetAutoAE(self, username=""):
    self.asid = ""
    self.getexp.clear()
    threading.Thread(target=self.DoSetAutoAE, args=(username,), name="%sDoSetAutoAE%d" %(self.title,self.number)).start()
  
  def DoSetAutoAE(self, uname):
    ctelnet = XCameraTelnet(ip=self.ip,username=uname,title=self.title)
    commit = ctelnet.commit
    cmdlist = ['rm -f /tmp/fuse_a/custom/asid.txt && sleep 1 && /tmp/fuse_a/custom/autoae.sh']
    msglist = ['[A/S/I/D]']
    ctelnet.RunCommand(cmdlist, msglist)
    while True:
      commit.wait(1)
...
#         self.dgain = 0
  ...
  def GetWebInstall(self):
    self.dlcomplete.clear()
    self.dlerror.clear()
    threading.Thread(target=self.DoGetWebInstall, name="%sDoGetWebInstall%d" %(self.title,self.number)).start()
  
  def DoGetWebInstall(self):
    md5_value = ""
    #url = 'https://github.com/stgvr/ponerinex/raw/master/xrelease/install.zip'
    url = 'https://github.com/stgvr/ponerinex/raw/master/xrelease/xuetan%20ae(fw1.2.13)%20preview%200.0.8.zip'
    try:
      response = urllib2.urlopen(url=url,timeout=30)
      total_size = int(response.info().getheader('Content-Length').strip())
      bytes_so_far = 0
      zipfile = __file__.replace(basename(__file__), "xdata/install.zip")
      with open(zipfile,'wb') as file:
        while True:
          ...
          if not chunk:
            break
          file.write(chunk)
      with open(zipfile, 'rb') as downfile:
        file_content = downfile.read()
  ...
  def GetWebVersion(self):
    self.webversion = ""
    self.getexp.clear()
    threading.Thread(target=self.DoGetWebVersion, name="%sDoGetWebVersion%d" %(self.title,self.number)).start()
  
  def DoGetWebVersion(self):
    url = 'https://github.com/stgvr/ponerinex/raw/master/xrelease/version.txt'
    webver = ""
    try:
      response = urllib2.urlopen(url=url,timeout=30)
      version = response.read()
      webver = version.split("<scriptversion>")[1].split("</scriptversion>")[0]
    finally:
      self.webversion = webver
      self.getexp.set()
    print "DoGetWebVersion %s" %self.webversion
  
  def GetAppVersion(self):
    verfile = __file__.replace(basename(__file__), "xdata/version.txt")
    with open(verfile, "r") as file:
      version = file.read()
    appver = ""
    try:
      appver = version.split("<scriptversion>")[1].split("</scriptversion>")[0]
    finally:
      return appver
      
  def GetCamVersion(self, username=""):
    self.camversion = ""
    self.getexp.clear()
    threading.Thread(target=self.DoGetCamVersion, args=(username,), name="%sDoGetCamVersion%d" %(self.title,self.number)).start()
    
  def DoGetCamVersion(self, uname):
    ctelnet = XCameraTelnet(ip=self.ip,username=uname,title=self.title)
    commit = ctelnet.commit
    cmdlist = ['if [ -e /tmp/fuse_a/custom/getver.sh ];then /tmp/fuse_a/custom/getver.sh;fi;']
    msglist = ['[VERSION]']
    ctelnet.RunCommand(cmdlist, msglist)
    while True:
      commit.wait(1)
      if commit.isSet():
        self.camversion = ctelnet.retvalue
        break
      if ctelnet.failure:
        self.camversion = ""
        break
    self.getexp.set()
    print "DoGetCamVersion %s" %self.camversion
  
  def CamScriptUpgrade(self, username=""):
    self.DebugLog("Camera Script Upgrade")
    self.setok.clear()
    self.getexp.clear()
    threading.Thread(target=self.DoCamScriptUpgrade, args=(username,), name="%sDoCamScriptUpgrade%d" %(self.title,self.number)).start()
  
  def DoCamScriptUpgrade(self, uname):
    print "DoCamScriptUpgrade ChangeDir /tmp/fuse_a"
    self.DebugLog("Change Dir to /tmp/fuse_a")
    self.ChangeDir('/tmp/fuse_a')
    i = 0
    while True:
      i += 1
      self.lsdir.wait(1)
      ...
      if self.lsdir.isSet():
        break
      if i > 60:
        self.DebugLog("Change Dir Timeout")
        self.getexp.set()
        return
    if self.status["pwd"] == "":
      self.getexp.set()
    ...
    zipfile = __file__.replace(basename(__file__), "xdata/install.zip")
    print "DoCamScriptUpgrade StartUpload", zipfile
    self.DebugLog("Upload %s" %zipfile)
    self.StartUpload(zipfile)
    i = 0
    while True:
      i += 1
      self.dlstop.wait(1)
      ...
      if self.dlstop.isSet():
        break
      if i > 60:
        self.DebugLog("Upload install.zip Timeout")
        self.getexp.set()
        return
    if not self.dlcomplete.isSet():
      self.DebugLog("Upload install.zip Error")
      self.getexp.set()
      return
    
    md5_value = self.MD5CheckSum(zipfile)
    if md5_value == "":
      self.DebugLog("MD5 Check Sum Error")
      self.getexp.set()
      return
    print "DoCamScriptUpgrade MD5CheckSum", md5_value
    self.DebugLog("MD5 Check Sum %s" %md5_value)
    
    self.DebugLog("Start to Upgrade Script")
    ctelnet = XCameraTelnet(ip=self.ip,username=uname,title=self.title)
    commit = ctelnet.commit
    cmdlist = ['mv -f /tmp/fuse_a/install.zip /tmp/install.zip;mkdir -p /tmp/install/;rm -f /tmp/install/*.*;unzip -o /tmp/install.zip -d /tmp/install/;chmod -R 777 /tmp/install/*.*;/tmp/install/upgrade.sh %s' %md5_value]
    msglist = ['[UPGRADE]']
    ctelnet.RunCommand(cmdlist, msglist)
    while True:
      commit.wait(1)
      if commit.isSet():
        print ctelnet.retvalue
        self.DebugLog("Upgrade Script Success")
        self.setok.set()
        break
      if ctelnet.failure:
        print "ERROR"
        self.DebugLog("Upgrade Script Error")
        self.DebugLog(ctelnet.debuglog)
        break
    self.getexp.set()
    #print "DoCamScriptUpgrade %s" %self.camversion
      ...
      while self.socketopen == 0:
        ...
        if self.msgbusy == 0:
          ...
          if allowsendout:
            ...
            # take photo
            if self.msgbusy in (769,16777220):
              self.photodelay = 0.0
              self.delaycounter = time.time()
              #print "start photo delay counter %d %s" %(self.msgbusy, self.delaycounter)
            # video record
            elif self.msgbusy == 513:
              self.videodelay = 0.0
              self.delaycounter = time.time()
              #print "start video delay counter %d %s" %(self.msgbusy, self.delaycounter)
            elif self.msgbusy == 1:
              self.photodelay = 0.0
              self.videodelay = 0.0
              self.delaycounter = time.time()
            if data.has_key("md5sum"):
              smsg = '{"token":%d,"msg_id":%d,"param":"%s","md5sum":"%s","offset":%d,"size":%d}' %(data["token"],data["msg_id"],data["param"],data["md5sum"],data["offset"],data["size"])
            elif data.has_key("type"):
  ...
  def JsonStatus(self, data):
    ...
    if data.has_key("param"):
      if data["type"] == "battery":
        ...
        if self.batlastvalue == 0 or self.batlastvalue < int(data["param"]):
          self.battime = self.batpercent * int(data["param"])
          self.batlast = 0
        elif self.batlastvalue > int(data["param"]):
          if self.batlast == 0:
            self.battime = self.batpercent * int(data["param"])
          else:
            self.battime = int((time.time() - self.batlast) / (self.batlastvalue - int(data["param"])) * int(data["param"]))
          self.batlast = time.time()
        self.batlastvalue = int(data["param"])
        print 'battery remain %d seconds' %self.battime
        ...
        if len(arr) == 6:
          ...
          self.dirtaken = arr[4]
        print 'photo_taken', self.dirtaken, self.filetaken
        ...
        Clock.unschedule(self.CardUsage)
      ...
      #{"msg_id":7,"type":"put_file_fail","param": 0}
        ...
        self.photodelay = time.time() - self.delaycounter
        #print "photo_delay %s" %self.photodelay
        self.DebugLog("photo_delay %s" %self.photodelay)
      else:
        self.status[data["type"]] = data["param"]
    else:
      print data
      if data["type"] == "start_video_record":
        self.videodelay = time.time() - self.delaycounter
        #print "video_delay %s" %self.videodelay
        self.DebugLog("video_delay %s" %self.videodelay)
        self.cambusy = False
        self.recording.set()
        if self.showtime:
          time.sleep(1)
        ...
        Clock.unschedule(self.CardUsage)
        Clock.schedule_interval(partial(self.CardUsage, "free"), 300)
        ...
        self.status["LOW_SPEED_CARD"] = 1
        self.DebugLog(data["type"])
        print "LOW_SPEED_CARD"
      elif data["type"] == "STORAGE_RUNOUT":
        self.status["STORAGE_RUNOUT"] = 1
        self.memory = -2
        Clock.unschedule(self.CardUsage)
        self.DebugLog(data["type"])
        print "STORAGE_RUNOUT"
        ...
        self.DebugLog(json.dumps(data,sort_keys=True))
        print '%s' %data
  '''
  normal rval = 0
  other rval:
  ...
  def JsonRval(self, data):
    ...
    if data["rval"] < 0:
      if data["msg_id"] == 1283:
        ...
        if data["rval"] == -17: #STORAGE_RUNOUT
          self.status["STORAGE_RUNOUT"] = 1
          self.memory = -2
    ...
    # vf stop
      ...
      if data["msg_id"] == 1:
        self.photodelay = time.time() - self.delaycounter
        self.videodelay = self.photodelay
        #print "send msg 1 delay %s" %self.videodelay
        self.DebugLog('status_delay %s' %self.photodelay)
      elif data["msg_id"] == 2:
        self.setok.set()
    ...
    # all config information
      ...
      self.setallok.set()
      if self.cfgdict.has_key("app_status"):
        if self.cfgdict["app_status"] in ("record","recording"):
          self.recording.set()
          self.SendMsg('{"msg_id":515}')
          Clock.unschedule(self.CardUsage)
          Clock.schedule_interval(partial(self.CardUsage, "free"), 300)
    ...
    #format card
      ...
      self.status["STORAGE_RUNOUT"] = 0
      self.status["LOW_SPEED_CARD"] = 0
      self.setok.set()
      if self.spacetotal == 0:
        self.CardUsage()
      else:
    ...
    # battery status
      ...
      if int(data["param"]) > 0:
        self.status["battery"] = data["param"]
        if self.status["adapter_status"] == "1":
          self.batlast = 0
          self.battime = self.batpercent * int(data["param"])
          self.batlastvalue = int(data["param"])
        else:
    ...
    # get file info
      ...
      if data.has_key("media_type") and data.has_key("resolution") and data.has_key("size") and data.has_key("duration"):
        if data["resolution"] <> "320x240":
          ...
          if self.status.has_key("STORAGE_RUNOUT"):
            if self.status["STORAGE_RUNOUT"] <> 1:
              if self.spacetotal == 0:
                self.CardUsage()
              else:
                self.CardUsage('free')
  ...
  def TakePhoto(self, type="precise quality"):
    if type == "precise quality":
      #don't use this flag for take photo
      #self.recording.clear() 
  ...
  def MD5CheckSum(self, filewithpath):
    md5_value = ""
    try:
      with open(filewithpath, 'rb') as checkfile:
        file_content = checkfile.read()
      md5_value = hashlib.md5(file_content).hexdigest()
    finally:
      return md5_value
...
# "md5sum": "d41d8cd98f00b204e9800998ecf8427e"
  ...
  def CardUsage(self, type="all", *args):
    if type == "total":
  ...
  def DebugLog(self, msg):
    logfile = __file__.replace(basename(__file__), "xdata/xlog%s.log" %(time.strftime('%Y%m%d')))
    try:
      with open(logfile,'a') as file:
        date_time = time.strftime('%Y-%m-%d %H:%M:%S')
        log_msg = '[%s] [%s] %s\n' %(date_time, self.ip, msg)
        #print 'DEBUG LOG: %s' %log_msg
        file.write(log_msg)
